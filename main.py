...
try:
    ...
    _VOCECHAT_ADAPTER_CLASS = VoceChatAdapter
    logger.info("RelayChatPlugin: Successfully imported VoceChatEvent and VoceChatAdapter.") # 技术性细节，保留英文
except ImportError:
    logger.info("RelayChatPlugin: VoceChatEvent 或 VoceChatAdapter 未找到。模拟事件将使用通用的 AstrMessageEvent。") # 针对中文用户调整
    class VoceChatEvent(AstrMessageEvent): pass
...
@register( "relaychat", "HikariFroya", "多Bot配置轮流连锁回复插件", "1.0.17" ) # 版本递增
class RelayChatPlugin(Star):
    ...
    def __init__(self, context: Context, config: AstrBotConfig):
        ...
        if hasattr(HistoryStorage, 'init'): HistoryStorage.init(self.config)
        ...
        logger.debug(f"RelayChatPlugin __init__: initial_min_delay = {self.initial_min_delay}, initial_max_delay = {self.initial_max_delay}")
        ...
        logger.debug(f"RelayChatPlugin __init__: chain_min_delay = {self.chain_min_delay}, chain_max_delay = {self.chain_max_delay}")
    ...
    async def __aexit__(self, exc_type, exc_val, exc_tb): 
        ...
        async with self.chain_scheduler_lock: 
            for task in list(self.active_chain_tasks.values()): 
                if task and not task.done(): task.cancel()
            self.active_chain_tasks.clear() 
    ...
    async def _common_message_handler(self, event: AstrMessageEvent, message_type_str: str):
        ...
        if not _is_chain_event and not bot_specific_config: return 
        if not bot_specific_config and _is_chain_event: logger.debug(f"RelayChatPlugin ({event_platform_id}): 收到连锁事件，但此平台未配置Bot。"); return
        elif not bot_specific_config: return
        ...
        logger.debug(f"{log_prefix}: 收到 {message_type_str}。连锁:{_is_chain_event}, 深度:{_chain_depth}, 上个回复者:{_last_replier_persona}, 原始消息ID:{_orig_mid}")
        if not _is_chain_event and self.initial_min_delay < self.initial_max_delay : 
            await asyncio.sleep(round(random.uniform(self.initial_min_delay, self.initial_max_delay), 1))
    ...
    async def _handle_event(self, event: AstrMessageEvent, bot_specific_config: Dict[str, Any], is_chain_event: bool, chain_depth: int, last_replier_persona: Optional[str]):
        ...
        if not is_chain_event:
            ...
            async with self.chain_scheduler_lock:
                task = self.active_chain_tasks.pop(original_user_mid_str, None)
                if task and not task.done(): task.cancel(); logger.info(f"{log_prefix}: 新用户消息，已取消原始消息ID {original_user_mid_str} 的连锁任务。")
        ...
        if not am_i_qualified: logger.debug(f"{log_prefix}: 不符合回复条件 (连锁且与上个回复者相同)。"); return
    ...
    async def _after_my_reply_sent(self, event: AstrMessageEvent):
        ...
        if hasattr(event, '_result') and event._result :
            ...
            elif hasattr(event._result, 'chain') and isinstance(event._result.chain, list): actual_reply_chain = event._result.chain
        if actual_reply_chain and len(actual_reply_chain) > 0:
            try: await HistoryStorage.process_and_save_bot_reply(event, actual_reply_chain, replied_bot_physical_id_that_just_spoke, replied_persona_name)
            ...
            if current_reply_depth < self.max_chain_depth:
                        ...
                        if chain_task_key not in self.active_chain_tasks or self.active_chain_tasks[chain_task_key].done():
                            self.active_chain_tasks[chain_task_key] = asyncio.create_task(
                                self._schedule_internal_chain_trigger(
                                    ...
                                    target_message_type = MessageType.GROUP_MESSAGE, replied_persona_name = replied_persona_name, 
                                    replied_bot_physical_id = replied_bot_physical_id_that_just_spoke, 
                                    target_bot_physical_id_for_self_id = target_bot_physical_id_for_target_event, 
                ...
                if original_user_mid_val: 
                    async with self.chain_scheduler_lock: 
                        ...
                        for k_pop in keys_to_pop: self.active_chain_tasks.pop(k_pop, None)
    ...
    async def _schedule_internal_chain_trigger(self, 
        ...
        await asyncio.sleep(round(random.uniform(self.chain_min_delay, self.chain_max_delay), 1 )) # 确保延迟是合理的
        ...
        try:
            ...
            else: new_msg_obj.group_id = None; new_msg_obj.session_id = target_session_id
            ...
            if _VOCECHAT_EVENT_AVAILABLE and target_platform_meta.name == "vocechat":
                target_adapter_instance = None
                if hasattr(self.context, 'platform_manager') and self.context.platform_manager:
                    if hasattr(self.context.platform_manager, 'get_platform_by_id'): target_adapter_instance = self.context.platform_manager.get_platform_by_id(target_platform_meta.id)
                    elif hasattr(self.context.platform_manager, 'get_insts'):
                        for inst in self.context.platform_manager.get_insts(): # type: ignore
                            if hasattr(inst, 'metadata') and inst.metadata and inst.metadata.id == target_platform_meta.id: target_adapter_instance = inst; break
        ...
        async with self.chain_scheduler_lock: self.active_chain_tasks.pop(chain_task_key, None)

def _cleanup_relay_extras(event: AstrMessageEvent): 
    event.set_extra("relay_is_chain", None); event.set_extra("relay_chain_depth", None); event.set_extra("relay_last_replier_persona", None)
    event.set_extra("relay_current_reply_config_for_hook", None); event.set_extra("relay_model_to_use", None)
    event.set_extra("relay_triggering_event_platform_meta_id", None); event.set_extra("relay_triggering_event_platform_meta_name", None)
    event.set_extra("relay_triggering_event_session_id", None); event.set_extra("relay_last_replier_physical_id", None)
    event.set_extra("relay_triggering_event_message_type", None) # 清理新增的 extra
